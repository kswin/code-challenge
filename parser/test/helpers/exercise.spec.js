...
var exerciseUtils = require('../../src/helpers/exercise');
...
describe('[Parser] Exercise helper functions', function(){
    describe('sanitizeExercise', function() {
        it('should sanitize question property into string', function(){
            ...
            var sanitizedExercise = exerciseUtils.sanitizeExercise(exercise);
    ...
    describe('isValidExercise', function(){
        it('should return false for invalid question value', function() {
            ...
            expect(exerciseUtils.isValidExercise(exercise)).to.be.false;
        });
        ...
        it('should return true for exercises with valid question, answer and distractors value', function() {
            ...
            expect(exerciseUtils.isValidExercise(exercise)).to.be.true;
        });
    });

    describe('getExerciseType', function(){
        it('should return correct exercise type based on question content', function(){
            expect(exerciseUtils.getExerciseType('What is 2 * 2?')).to.be.equal('multiplication');
            expect(exerciseUtils.getExerciseType('What is 2 + 2?')).to.be.equal('addition');
            expect(exerciseUtils.getExerciseType('What is 2 - 2?')).to.be.equal('subtraction');
            expect(exerciseUtils.getExerciseType('What is 2 / 2?')).to.be.equal('division');
        });
    });
});